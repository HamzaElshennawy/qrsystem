  ...
  const testAllPermissions = async () => {
    ...
    try {
      ...
      try {
        ...
        setResult(prev => prev + `✅ Compounds read: ${compounds.length} found\n`);
      } catch (error: any) {
        setResult(prev => prev + `❌ Compounds read failed: ${error.message}\n`);
      }
      ...
      try {
        ...
        setResult(prev => prev + `✅ Compound created: ${compoundId}\n`);
        
        // Clean up
        await firestoreService.delete('compounds', compoundId);
        setResult(prev => prev + `✅ Test compound cleaned up\n`);
      } catch (error: any) {
        setResult(prev => prev + `❌ Compound creation failed: ${error.message}\n`);
      }
      ...
      try {
        ...
        if (compounds.length > 0) {
          ...
          const ownerId = await firestoreService.owners.create(testOwner);
          setResult(prev => prev + `✅ Owner created: ${ownerId}\n`);
          
          // Clean up
          await firestoreService.delete('owners', ownerId);
          setResult(prev => prev + `✅ Test owner cleaned up\n`);
        } else {
          setResult(prev => prev + `⚠️ No compounds found to test owner creation\n`);
        }
      } catch (error: any) {
        setResult(prev => prev + `❌ Owner creation failed: ${error.message}\n`);
      }
      ...
      <Card className="max-w-2xl mx-auto">
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-blue-50 rounded-lg">
            ...
            <p>{authService.getCurrentUser()?.email || 'Not logged in'}</p>
          </div>

          <Button onClick={testAllPermissions} disabled={loading} className="w-full">
          ...
          <div className="mt-6 p-4 bg-yellow-50 rounded-lg">
            ...
            <ol className="text-sm list-decimal list-inside space-y-1">
              ...
              <li>Create the missing index (click the link in the error)</li>
              <li>Wait a few minutes for index creation to complete</li>
            </ol>
          </div>
        </CardContent>
      </Card>
