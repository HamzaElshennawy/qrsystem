...
import { firestoreService, type QRCode, type User, type Compound } from '@/firebase/firestore';
import { onAuthStateChanged } from 'firebase/auth';
import { auth } from '@/firebase/firebaseConfig';
import { QrCode, RefreshCw, Plus, Clock, User, Building, Home } from 'lucide-react';
...
export default function OwnerDashboardPage() {
  ...
  const [entranceQRCodes, setEntranceQRCodes] = useState<Map<string, EntranceQRCode>>(new Map());
  const [guestQRCodes, setGuestQRCodes] = useState<GuestQRCode[]>([]);
  const [loading, setLoading] = useState(true);
  const [qrRefreshTimers, setQrRefreshTimers] = useState<Map<string, NodeJS.Timeout>>(new Map());
  ...
  const [availableCompounds, setAvailableCompounds] = useState<{id: string; name: string; address: string}[]>([]);
  ...
  const loadOwnerProfile = async () => {
    try {
      ...
      const response = await fetch(`/api/owner/profile?${params.toString()}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const loadEntranceQRCode = async (compoundId: string) => {
    try {
      ...
      const response = await fetch(`/api/owner/qr/entrance?${params.toString()}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const createOwnerProfile = async () => {
    if (!createForm.firstName || !createForm.lastName || !createForm.email || !createForm.compoundId) {
  ...
  if (ownerProfiles.length === 0) {
    return (
      <div className="p-4 space-y-6">
        <Alert>
          <AlertDescription>
            No owner profile found. You can either contact your compound administrator to set up your account, or create your own profile if you have a compound ID.
          </AlertDescription>
        </Alert>
          ...
          <CardContent>
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>Create Owner Profile</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setCreateForm(prev => ({ ...prev, propertyUnit: e.target.value }))}
                      placeholder="e.g., Unit A1, Apartment 5B"
                    />
                  </div>
                  
                  <div className="space-y-2">
                    <Label htmlFor="compoundId">Select Compound *</Label>
                    <select
                      id="compoundId"
                      value={createForm.compoundId}
                      onChange={(e) => setCreateForm(prev => ({ ...prev, compoundId: e.target.value }))}
                      className="w-full p-2 border rounded-md"
                    >
                      <option value="">Select a compound</option>
                      {availableCompounds.map((compound) => (
                        <option key={compound.id} value={compound.id}>
                          {compound.name} - {compound.address}
                        </option>
                      ))}
                    </select>
                    <p className="text-xs text-muted-foreground">
                      If your compound is not listed, contact your administrator
                    </p>
                  </div>
                  
                  <Button
                    onClick={createOwnerProfile}
                    disabled={creatingProfile || !createForm.firstName || !createForm.lastName || !createForm.email || !createForm.compoundId}
                    className="w-full"
                  >
                    {creatingProfile ? (
                      <>
    ...
    <div className="p-4 space-y-6">
            ...
            {ownerProfiles.map((owner) => (
              <div key={owner.id} className="border rounded-lg p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{owner.compound.name}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Home className="h-4 w-4" />
                  <span>Unit: {owner.propertyUnit || 'Not specified'}</span>
                </div>
                <div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {ownerProfiles.map((owner) => {
              const entranceQR = entranceQRCodes.get(owner.compound.id!);
              return (
                <div key={owner.compound.id} className="border rounded-lg p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold">{owner.compound.name}</h3>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => refreshEntranceQR(owner.compound.id!)}
                    >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="text-center space-y-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => downloadQRCode(entranceQR.dataURL, `entrance-qr-${owner.compound.name}.png`)}
                        >
                          Download QR Code
                        </Button>
                      </div>
      ...
      <Card>
        ...
        <CardContent>
          <Dialog open={isGuestDialogOpen} onOpenChange={setIsGuestDialogOpen}>
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>Generate Guest QR Code</DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  >
                    <option value="">Select a compound</option>
                    {ownerProfiles.map((owner) => (
                      <option key={owner.compound.id} value={owner.compound.id}>
                        {owner.compound.name}
                      </option>
                    ))}
                  </select>
                ...
                <Button
                  ...
                  disabled={generatingGuestQR || !selectedCompoundId || !guestName}
                  className="w-full"
                >
                  {generatingGuestQR ? (
                    <>
          ...
          <CardContent>
            <div className="space-y-4">
              {guestQRCodes.map((guestQR) => (
                <div key={guestQR.id} className="border rounded-lg p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold">{guestQR.guestName}</h3>
                      <p className="text-sm text-muted-foreground">
                        Expires: {new Date(guestQR.expiresAt).toLocaleString()}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => downloadQRCode(guestQR.dataURL, `guest-qr-${guestQR.guestName}.png`)}
                    >
                      Download
                    </Button>
                  </div>
