...
export default function OwnerLoginPage() {
    ...
    const checkAuthenticationStatus = useCallback(async () => {
        // If we've detected the phone is not registered, do not run the device check
        // which can overwrite the current flow.
        if (phoneNotRegistered) return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                // If server responded that phone is not registered by admin, show a friendly message
                if (response.status === 404) {
                    try {
                        const err = await response.json();
                        if (err?.notRegisteredByAdmin) {
                            setError(
                                "Your phone number isn't registered by an administrator yet."
                            );
                            // Don't set owner to avoid showing admin profile
                            setOwner(null);
                            // Mark persistent flag so initial check won't override
                            setPhoneNotRegistered(true);
                            // Show explicit not-registered state so user can't create a password
                ...
                // Default: user not authenticated, start with OTP
                setStep("otp");
            }
        } catch (e: unknown) {
            setError(
    ...
    }, [deviceFingerprint, userAgent, phoneNotRegistered]);
    ...
    const verifyOtp = async () => {
        try {
            setLoading(true);
            setError("");
            const userCredential = await authService.confirmOtp(
                confirmationRef.current!,
                code
            );
            const uid = userCredential?.user?.uid;
            const phoneFromFirebase = userCredential?.user?.phoneNumber || null;
            ...
            if (response.ok) {
                ...
                // If phone is not registered by admin, show a dedicated message and stop showing admin profile
                if (response.status === 404) {
                    try {
                        const err = await response.json();
                        if (err?.notRegisteredByAdmin) {
                            ...
                            setOwner(null);
                            setPhoneNotRegistered(true);
                            // Do NOT allow creating a password when there's no profile
                            setStep("not-registered");
                            return;
                        }
                    } catch {
    ...
    const setupPassword = async () => {
        try {
            ...
            setError(
                e instanceof Error ? e.message : "Failed to set up password"
            );
        } finally {
            setLoading(false);
        }
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            ...
                            {step === "setup-password" &&
                                "Set up your password for future logins"}
                            {step === "password-login" &&
                        ...
                        {step === "not-registered" && (
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Alert>
                                        <AlertDescription>
                                            ...
                                            by an administrator yet. Please
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        onClick={async () => {
                                            // Sign out any firebase auth state and clear local auth info,
                                            // then let user try OTP again.
                                            try {
                                    ...
                                    <Button
                                        variant="outline"
                                        onClick={async () => {
                                            // Clear auth state and return to initial check which will re-run
                                            try {
                                                await authService.signOut();
                                            } catch {
                                                // ignore
                                            ...
                                            setError("");
                                            setStep("initial");
                                            // Explicitly re-run the device/auth check
                                            try {
