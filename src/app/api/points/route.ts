...
export async function GET(request: NextRequest) {
  try {
    ...
    if (compoundId) {
      ...
      entryPoints = await firestoreService.entryPoints.getByCompound(compoundId);
    } else {
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to fetch entry points' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const entryPointId = await firestoreService.entryPoints.create(entryPointData);
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to create entry point' },
      { status: 500 }
    );
  }
}
