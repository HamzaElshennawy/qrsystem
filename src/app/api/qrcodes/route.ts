  ...
  try {
    ...
    if (ownerId) {
      ...
      for (const compound of compounds) {
        const compoundQrCodes = await firestoreService.qrCodes.getByCompound(compound.id!);
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingQrCodes = await firestoreService.qrCodes.getByOwner(ownerId);
    ...
    // Generate QR code
    const qrCodeId = await qrService.createAndSaveQRCode(owner, compoundId, currentUser.uid);
