...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ownerIds && ownerIds.length > 0) {
      ...
      const allOwners = await firestoreService.owners.getByCompound(compoundId);
      const existingQrCodes = await firestoreService.qrCodes.getByCompound(compoundId);
      const existingOwnerIds = existingQrCodes.map(qr => qr.ownerId);
      
      owners = allOwners.filter(owner => !existingOwnerIds.includes(owner.id!));
    }
    ...
    // Generate QR codes
    qrCodeIds = await qrService.bulkGenerateQRCodes(owners, compoundId, currentUser.uid);
