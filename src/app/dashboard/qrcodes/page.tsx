...
import { firestoreService, type QRCode, type Compound, type Owner } from '@/firebase/firestore';
...
export default function QRCodesPage() {
  ...
  const handleGenerateQRCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate QR code
      await qrService.createAndSaveQRCode(owner, selectedCompound!.id!, currentUser.uid);
  ...
  const handleBulkGenerate = async () => {
    try {
      ...
      const ownersWithoutQr = owners.filter(owner => 
        !qrCodes.some(qr => qr.ownerId === owner.id)
      );
      ...
      await qrService.bulkGenerateQRCodes(ownersWithoutQr, selectedCompound!.id!, currentUser.uid);
  ...
  const handlePreviewQR = async (ownerId: string) => {
    try {
      ...
      const qrResult = await qrService.generateOwnerQRCode(owner, selectedCompound!.id!);
  ...
  const handleDownloadQR = async (qrCode: QRCode) => {
    try {
      ...
      const qrResult = await qrService.generateOwnerQRCode(owner, qrCode.compoundId);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-slate-600 dark:text-slate-400">Loading QR codes...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <Dialog open={isBulkGenerateDialogOpen} onOpenChange={setIsBulkGenerateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Generate QR codes for all owners who don't have one yet.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                <div className="p-4 bg-muted rounded-lg">
                  <p className="text-sm">
                    <strong>{ownersWithoutQr.length}</strong> owners don't have QR codes yet.
                  </p>
                  {ownersWithoutQr.length > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">
                      QR codes will be generated for all of them.
                    </p>
                  )}
                </div>
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="outline" onClick={() => setIsBulkGenerateDialogOpen(false)}>
                    Cancel
                  </Button>
                  <Button 
                  ...
                  >
                    Generate {ownersWithoutQr.length} QR Codes
                  </Button>
                </div>
              </div>
            </DialogContent>
          ...
          <Dialog open={isGenerateDialogOpen} onOpenChange={setIsGenerateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Generate a QR code for a specific property owner.
                </DialogDescription>
              </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Owners Without QR</CardTitle>
            <Users className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{ownersWithoutQr.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {qrCodes.reduce((total, qr) => total + (qr.metadata?.scanCount || 0), 0)}
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <TableBody>
                {filteredQrCodes.map((qrCode) => (
                  <TableRow key={qrCode.id}>
                    ...
                    <TableCell>{qrCode.metadata?.scanCount || 0}</TableCell>
                    <TableCell>
                      {qrCode.createdAt?.toDate().toLocaleDateString()}
                    </TableCell>
                    <TableCell>
                      <div className="flex gap-2">
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={() => handlePreviewQR(qrCode.ownerId)}
                        >
        ...
        <Dialog open={!!previewQR} onOpenChange={() => setPreviewQR('')}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                This is how the QR code will appear when printed.
              </DialogDescription>
            </DialogHeader>
            <div className="flex justify-center">
              <img src={previewQR} alt="QR Code Preview" className="max-w-full h-auto" />
            </div>
          </DialogContent>
        </Dialog>
      )}
