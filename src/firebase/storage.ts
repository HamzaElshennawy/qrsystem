...
export const storageService = {
  ...
  qrCodes: {
    uploadQRCode: async (
      ...
      await storageService.uploadFile(file, path, { contentType: 'image/png' });
      return await storageService.getDownloadURL(path);
    },

    deleteQRCode: async (compoundId: string, ownerId: string, fileName: string): Promise<void> => {
      const path = `qrcodes/${compoundId}/${ownerId}/${fileName}`;
      await storageService.deleteFile(path);
    },

    getQRCodeURL: async (compoundId: string, ownerId: string, fileName: string): Promise<string> => {
      const path = `qrcodes/${compoundId}/${ownerId}/${fileName}`;
      return await storageService.getDownloadURL(path);
    },
  },
  ...
  csv: {
    uploadCSV: async (
      ...
      await storageService.uploadFile(file, path, { contentType: 'text/csv' });
      return await storageService.getDownloadURL(path);
    },

    deleteCSV: async (compoundId: string, fileName: string): Promise<void> => {
