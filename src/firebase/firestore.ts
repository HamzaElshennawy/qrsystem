...
export const firestoreService = {
  ...
  create: async <T>(collectionName: string, data: Omit<T, 'id'>): Promise<string> => {
    try {
  ...
  read: async <T>(collectionName: string, docId: string): Promise<T | null> => {
    try {
  ...
  update: async <T>(collectionName: string, docId: string, data: Partial<T>): Promise<void> => {
    try {
      const docRef = doc(db, collectionName, docId);
  ...
  compounds: {
    create: async (data: Omit<Compound, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  ...
  owners: {
    create: async (data: Omit<Owner, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
      return firestoreService.create<Owner>('owners', data);
    },

    getByCompound: async (compoundId: string): Promise<Owner[]> => {
      const owners = await firestoreService.query<Owner>(
        'owners',
    ...
    bulkCreate: async (owners: Omit<Owner, 'id' | 'createdAt' | 'updatedAt'>[]): Promise<string[]> => {
      const batch = [];
      const ids: string[] = [];
  ...
  qrCodes: {
    create: async (data: Omit<QRCode, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
      return firestoreService.create<QRCode>('qrcodes', data);
    },

    getByCompound: async (compoundId: string): Promise<QRCode[]> => {
      const qrCodes = await firestoreService.query<QRCode>(
        'qrcodes',
        [where('compoundId', '==', compoundId)]
      );
      // Sort manually to avoid index issues
    ...
    getByOwner: async (ownerId: string): Promise<QRCode[]> => {
      return firestoreService.query<QRCode>(
        'qrcodes',
  ...
  entryPoints: {
    create: async (data: Omit<EntryPoint, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
